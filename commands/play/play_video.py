...
async def play_video(ctx, player_queue, bot):
    ...
    def after_playing(error):
        if not player_queue.is_empty():
            asyncio.run_coroutine_threadsafe(next_video(ctx, player_queue, bot), bot.loop).result()
...
async def next_video(ctx, player_queue, bot):
